<div class="col-lg-3">
	<div class="list-group">
		...
		<a href="http://localhost/bookshop/?r=site/profile&menu_point=cur_orders" class="list-group-item active">
...
<div class="col-lg-9 order_info_block">
	...
	<?php for ($i = 0; $i < $c_orders; $i++): ?>
		<div id="a_profile_cur_order_bloc_<?=$orders_info[$i]['id_order']?>">
			...
			<a href="http://localhost/bookshop/?r=site/profile&menu_point=new_orders&action=go_history_order&id_order=<?=$orders_info[$i]['id_order']?>" class="btn btn-success ajax_link">Отправить в историю</a>
			<hr/>
		</div>
	<?php endfor; ?>
	<?php if($c_orders == 0): ?>
		На данный момент нет заказов в обработке.
	<?php endif; ?>
</div>